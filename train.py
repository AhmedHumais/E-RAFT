...
def train(args):
    ...
    mvsec_dataset=MVSECDataset(Path('/media/ahmed/drive1/flow-data/mvsec20/MVSECGraphDatset/'))
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            optimizer.zero_grad()
            graph_data = [x.cuda() for x in data_blob]
            
            flow = graph_data[0].y[0]
            valid = graph_data[1].y[1]
            ...
            _, flow_predictions = model(graph_data, iters=args.iters)            
